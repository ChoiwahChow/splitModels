...
private:
	...
	static void hash_model(std::unordered_map<std::string, int>& blocks, const std::string& model_str,
			int& next_key, std::vector<std::vector<int>>& all_inv_vec, const std::string& output_file_prefix, const std::string& mace_filter,
			std::vector<std::vector<std::string>>& interps, int max_model_count_in_bucket = 1000000);
	...
	static int calc_all_invariants(std::string& in_file, int domain_size, int& num_models, int starting_seed, int num_random,
			...
			std::vector<std::vector<std::vector<int>>>& all_inv_vec, bool no_basic_invariants = false);
...
public:
	...
	static int bucketing(std::string& in_file, int domain_size, int& num_ops, int num_input_models,
			const std::string& output_file_prefix, const std::string& mace_filter, std::vector<std::vector<std::string>>& interps);
